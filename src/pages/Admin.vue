...
async function loadData() {
  ...
  try { const [teachersRes, statsRes] = await Promise.all([api.get('/admin/teachers'), api.get('/admin/stats')]); teachers.value = teachersRes.data.teachers; stats.value = statsRes.data.stats; dailyStats.value = statsRes.data.dailyStats } catch (e: any) { toast.error(e.response?.data?.error || '加载失败') } finally { isLoading.value = false }
}
  ...
  <div class="min-h-screen bg-gradient-to-b from-orange-50 to-amber-50 flex flex-col">
    ...
    <main v-else class="flex-1 max-w-4xl mx-auto p-4 space-y-6 w-full">
      <div class="grid grid-cols-2 md:grid-cols-5 gap-3">
        ...
        <div class="bg-white rounded-xl shadow-sm p-4 text-center"><div class="text-2xl font-bold text-green-500">{{ stats?.students || 0 }}</div><div class="text-sm text-gray-500">学生数</div></div>
        <div class="bg-white rounded-xl shadow-sm p-4 text-center"><div class="text-2xl font-bold text-purple-500">{{ stats?.evaluations || 0 }}</div><div class="text-sm text-gray-500">评价数</div></div>
        <div class="bg-white rounded-xl shadow-sm p-4 text-center"><div class="text-2xl font-bold text-rose-500">{{ stats?.todayEvaluations || 0 }}</div><div class="text-sm text-gray-500">今日评价</div></div>
      </div>
      <div class="bg-white rounded-xl shadow-sm p-4">
        <h3 class="font-bold text-gray-700 mb-3">📊 最近7天评价</h3>
        <div class="flex gap-2 overflow-x-auto pb-2">
          <div v-for="day in dailyStats" :key="day.date" class="flex-shrink-0 bg-gradient-to-b from-orange-50 to-amber-50 rounded-lg p-3 text-center min-w-[80px]"><div class="text-xs text-gray-500">{{ day.date.slice(5) }}</div><div class="text-lg font-bold text-orange-500">{{ day.count }}</div></div>
        </div>
      </div>
      <div class="bg-white rounded-xl shadow-sm overflow-hidden">
        <div class="p-4 border-b border-gray-100"><h3 class="font-bold text-gray-700">👨‍🏫 老师列表 ({{ teachers.length }} 人)</h3></div>
          ...
          <div v-for="teacher in teachers" :key="teacher.id" class="hover:bg-gray-50">
            <div class="p-4 flex items-center justify-between cursor-pointer" @click="toggleTeacher(teacher.id)">
              <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-full bg-gradient-to-r from-orange-400 to-pink-500 flex items-center justify-center text-white font-bold">{{ teacher.username.charAt(0).toUpperCase() }}</div>
                <div><div class="font-medium text-gray-800 flex items-center gap-2">{{ teacher.username }}<span v-if="teacher.isAdmin" class="text-xs bg-orange-100 text-orange-600 px-2 py-0.5 rounded-full">管理员</span></div><div class="text-sm text-gray-500">注册于 {{ formatDate(teacher.createdAt) }}</div></div>
              </div>
              <div class="flex items-center gap-4 text-sm">
                ...
                <div class="text-gray-400"><span class="inline-block transition-transform duration-200" :class="expandedTeacher === teacher.id ? 'rotate-180' : ''">▼</span></div>
              </div>
            </div>
              ...
              <div v-if="expandedTeacher === teacher.id && teacher.classes.length > 0" class="bg-gray-50 px-4 pb-4">
                <div class="pt-2 space-y-2">
                  <div v-for="cls in teacher.classes" :key="cls.id" class="bg-white rounded-lg p-3 flex items-center justify-between">
                    <div class="flex items-center gap-2"><span class="text-lg">📚</span><span class="font-medium text-gray-700">{{ cls.name }}</span></div>
                    <div class="flex items-center gap-4 text-sm"><span class="text-green-600">{{ cls.student_count }} 人</span><span class="text-purple-600">{{ cls.eval_count }} 条评价</span></div>
                  </div>
                </div>
              </div>
            </Transition>
