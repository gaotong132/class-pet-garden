...
async function addStudent() {
  ...
  try {
    await api.post('/students', { classId: currentClass.value!.id, name: newStudentName.value.trim(), studentNo: newStudentNo.value.trim() || null })
    toast.success('添加成功')
...
async function saveEdit() {
  ...
  try {
    await api.put(`/students/${editingStudent.value.id}`, { name: editName.value.trim(), studentNo: editStudentNo.value.trim() || null })
    toast.success('保存成功')
...
async function deleteStudent(id: string) {
  showConfirm({
    ...
    onConfirm: async () => {
      try { await api.delete(`/students/${id}`); toast.success('删除成功'); selectedIds.value.delete(id); localStorage.setItem('pet-garden-data-version', Date.now().toString()); loadStudents() }
...
async function deleteSelected() {
  ...
  showConfirm({
    ...
    onConfirm: async () => {
      try { await api.post('/students/batch-delete', { ids: Array.from(selectedIds.value) }); toast.success(`已删除 ${selectedIds.value.size} 名学生`); selectedIds.value.clear(); localStorage.setItem('pet-garden-data-version', Date.now().toString()); loadStudents() }
      catch (error) { toast.error('删除失败') }
    }
  })
}
...
async function importStudents() {
  ...
  const data = lines.map(line => { const parts = line.trim().split(/[,，;；\t\s]+/); return { name: parts[0]?.trim() || '', studentNo: parts[1]?.trim() || null } }).filter(s => s.name)
  if (data.length === 0) { toast.warning('没有有效的学生数据'); return }
  try { await api.post('/students/import', { classId: currentClass.value!.id, students: data }); toast.success(`成功导入 ${data.length} 名学生`); importText.value = ''; showImportForm.value = false; localStorage.setItem('pet-garden-data-version', Date.now().toString()); loadStudents() }
  catch (error) { toast.error('导入失败') }
}

function openTagModal(student: Student | null) { taggingStudent.value = student; showTagModal.value = true }
...
async function toggleTag(tag: Tag) {
  ...
  try {
    if (isApplied) { await api.post('/tags/batch-remove', { studentIds, tagId: tag.id }); toast.success(`已为 ${studentIds.length} 名学生移除标签`) }
    else { await api.post('/tags/batch-add', { studentIds, tagId: tag.id }); toast.success(`已为 ${studentIds.length} 名学生添加标签`) }
    for (const id of studentIds) await loadSingleStudentTags(id)
  } catch (error: any) { toast.error(error.response?.data?.error || '操作失败') }
}

function isTagApplied(tag: Tag): boolean {
  const studentIds = taggingStudent.value ? [taggingStudent.value.id] : Array.from(selectedIds.value)
  if (studentIds.length === 0) return false
  return studentIds.every(id => (studentTags.value.get(id) || []).some(t => t.id === tag.id))
}

watch(currentClass, (newClass) => { if (newClass) loadStudents(); else students.value = [] })

onMounted(async () => { await loadTags(); if (currentClass.value) await loadStudents(); else isLoading.value = false })
onActivated(() => { syncCurrentClass(); loadTags() })
</script>
        ...
        <div class="flex items-center justify-between mb-6">
          ...
          <div class="flex items-center gap-2 flex-wrap justify-end">
            <template v-if="selectedIds.size > 0">
              ...
              <button @click="deleteSelected" class="px-4 py-2 text-sm text-red-500 hover:bg-red-50 border border-red-200 rounded-xl font-medium transition-colors">🗑️ 删除 ({{ selectedIds.size }})</button>
            </template>
        ...
        <div v-else class="bg-white rounded-2xl shadow-sm overflow-hidden">
          ...
          <div v-for="student in filteredStudents" :key="student.id" class="grid grid-cols-12 gap-4 px-4 py-3 border-b border-gray-50 hover:bg-gray-50/50 transition-colors items-center" :class="{ 'bg-orange-50/30': selectedIds.has(student.id) }">
            ...
            <template v-if="editingStudent?.id === student.id">
              <div class="col-span-2"><input v-model="editName" type="text" class="w-full border-2 border-gray-200 rounded-lg px-2 py-1 text-sm focus:outline-none focus:border-orange-400" @keyup.enter="saveEdit" @keyup.escape="cancelEdit" /></div>
              <div class="col-span-2"><input v-model="editStudentNo" type="text" class="w-full border-2 border-gray-200 rounded-lg px-2 py-1 text-sm focus:outline-none focus:border-orange-400" @keyup.enter="saveEdit" @keyup.escape="cancelEdit" /></div>
              <div class="col-span-3"></div><div class="col-span-1 text-sm text-gray-600">{{ student.total_points }}</div>
              <div class="col-span-2 text-right"><button @click="saveEdit" class="text-green-500 hover:text-green-600 text-sm font-medium px-2 py-1">保存</button><button @click="cancelEdit" class="text-gray-400 hover:text-gray-600 text-sm font-medium px-2 py-1">取消</button></div>
            </template>
            <template v-else>
